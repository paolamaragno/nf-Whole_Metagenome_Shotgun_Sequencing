...
process BOWTIE2_SAMTOOLS_TWO_INDEX {
	...
	input:
	path(bowtie2_genome1_idx_folder)
	path(bowtie2_genome2_idx_folder)
	tuple val(sample_id), path(filtered_fastq1), path(filtered_fastq2)
	tuple val(name), path(fastq1), path(fastq2), val(host_genome_for_mapping)
	...
	script:
	"""
	if ${host_genome_for_mapping} == 'genome1'; then
                bowtie2 -x ${bowtie2_genome1_idx_folder}/${params.idx_genome_1} -1 ${filtered_fastq1} -2 ${filtered_fastq2} \
                -S ${sample_id}.sam --very-sensitive-local -p ${task.cpus}
        else
            	bowtie2 -x ${bowtie2_genome2_idx_folder}/${params.idx_genome_2} -1 ${filtered_fastq1} -2 ${filtered_fastq2} \
                -S ${sample_id}.sam --very-sensitive-local -p ${task.cpus}
        fi
